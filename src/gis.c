...
static int gis_read_record(FILE *fp, FILE *fp_idx, int type, double *bound, gisd_t **data) {
    gisd_t *p, *next;
    uint8_t buff[16];
...
extern int gis_read(const char *file, gis_t *gis, int layer) {
    ...
    /* read header */
    if ((type1 = read_shape_head(fp)) < 0 || (type2 = read_shape_head(fp_idx)) < 0 || type1 != type2) {
