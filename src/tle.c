...
static void SGP4_STR3(double tsince, const tled_t *data, double *rs) {
    ...
    c2 =
        coef1 * xnodp * (aodp * (1.0 + 1.5 * etasq + eeta * (4.0 + etasq)) + 0.75 * CK2 * tsi / psisq * x3thm1 * (8.0 + 3.0 * etasq * (8.0 + etasq)));
    c1     = bstar * c2;
    sinio  = sin(xincl);
...
extern int tle_read(const char *file, tle_t *tle) {
    ...
    while (fgets(buff, sizeof(buff), fp)) {
        ...
        if (buff[0] == '1' && checksum(buff)) {
            ...
            /* decode TLE line 1 */
            if (decode_line1(buff, &data))
                line = 1;
        } else if (line == 1 && buff[0] == '2' && checksum(buff)) {

            /* decode TLE line 2 */
            if (!decode_line2(buff, &data))
                continue;
    ...
    if (tle->n > 0)
        qsort(tle->data, tle->n, sizeof(tled_t), cmp_tle_data);
    return 1;
}
...
extern int tle_name_read(const char *file, tle_t *tle) {
    ...
    while (fgets(buff, sizeof(buff), fp)) {
        ...
        for (i = 0; i < tle->n; i++) {
            if (!strcmp(tle->data[i].satno, satno) || !strcmp(tle->data[i].desig, desig))
                break;
        }
        if (i >= tle->n) {
 ...
 *-----------------------------------------------------------------------------*/
extern int tle_pos(gtime_t time, const char *name, const char *satno, const char *desig, const tle_t *tle, const erp_t *erp, double *rs) {
    gtime_t tutc;
