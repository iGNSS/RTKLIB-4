...
static void convcode(int rnxver, int sys, char *type) {
    if (rnxver >= 212 && (sys == SYS_GPS || sys == SYS_QZS || sys == SYS_SBS) && !strcmp(type + 1, "1C")) { /* L1C/A */
        ...
        strcpy(type + 1, "D");
    } else if (sys == SYS_CMP && (!strcmp(type + 1, "2I") || !strcmp(type + 1, "2Q") || !strcmp(type + 1, "2X"))) { /* B1_2 */
        strcpy(type + 1, "2");
    } else if (!strcmp(type, "C1P") || !strcmp(type, "C1W") || !strcmp(type, "C1Y") || !strcmp(type, "C1N")) { /* L1P,P(Y) */
        strcpy(type, "P1");
    } else if (!strcmp(type, "C2P") || !strcmp(type, "C2W") || !strcmp(type, "C2Y") || !strcmp(type, "C2N") || !strcmp(type, "C2D")) { /* L2P,P(Y) */
        strcpy(type, "P2");
    } else {
        type[2] = '\0';
    }
...
static void unsetopt_file(rnxopt_t *opt) {
    ...
    for (i = MAXCOMMENT - 1; i >= 0 && !brk; i--) {
        if (!*opt->comment[i])
            continue;
        if (!strncmp(opt->comment[i], "format: ", 8))
            brk = 1;
        *opt->comment[i] = '\0';
    }
}
...
static void setopt_phshift(rnxopt_t *opt) {
    ...
    for (i = 0; i < NSATSYS; i++)
        for (j = 0; j < opt->nobs[i]; j++) {
            ...
            if (navsys[i] == SYS_GPS) {
                if (code == CODE_L1S || code == CODE_L1L || code == CODE_L1X || code == CODE_L1P || code == CODE_L1W || code == CODE_L1N) {
                    opt->shift[i][j] = 0.25; /* +1/4 cyc */
                } else if (code == CODE_L2C || code == CODE_L2S || code == CODE_L2L || code == CODE_L2X || code == CODE_L5Q) {
                    opt->shift[i][j] = -0.25; /* -1/4 cyc */
                }
...
static void setopt_sta(const strfile_t *str, rnxopt_t *opt) {
    ...
    for (p = str->stas; p; p = p->next) {
        if (!p->next)
            break;
        if (opt->ts.time && timediff(p->next->te, opt->ts) < 0.0)
            break;
    }
    if (p) {
        sta = &p->sta;
...
static void update_stas(strfile_t *str) {
    ...
    if (!str->stas || str->stas->staid != str->staid) { /* station ID changed */
        if (!(p = (stas_t *) calloc(sizeof(stas_t), 1)))
            return;
        p->staid = str->staid;
        p->ts = p->te = str->time;
        p->next       = str->stas;
...
static void dump_stas(const strfile_t *str) {
    ...
    trace(2, "# %17s %19s %5s %6s %16s %16s %12s %13s %9s %2s %6s %6s %6s\n", "TIME", "STAID", "MARKER", "ANTENNA", "RECEIVER", "LATITUDE",
          "LONGITUDE", "HIGHT", "DT", "DEL1", "DEL2", "DEL3");
    ...
    for (p = str->stas; p; p = p->next) {
        ...
        trace(2,
              ...
              "%6.3f %6.3f %6.3f\n",
              s1, s2, p->staid, p->sta.name, p->sta.antdes, p->sta.rectype, pos[0] * R2D, pos[1] * R2D, pos[2], p->sta.deltype, p->sta.del[0],
              p->sta.del[1], p->sta.del[2]);
    }
#endif
}
...
static int add_halfc(strfile_t *str, int sat, int idx, gtime_t time) {
    ...
    if (!(p = (halfc_t *) calloc(sizeof(halfc_t), 1)))
        return 0;
    p->ts = p->te            = time;
    p->stat                  = 0;
...
static void update_halfc(strfile_t *str, obsd_t *obs) {
    ...
    for (i = 0; i < NFREQ + NEXOBS; i++) {
        ...
        if (!str->halfc[sat - 1][i]) {
            if (!add_halfc(str, sat, i, obs->time))
                continue;
        }
    ...
    for (m = 0; m < nf && !abort; m++) {
        ...
        while ((type = input_strfile(str)) >= -1) {
            if (opt->ts.time && timediff(str->time, opt->ts) < -opt->ttol)
                continue;
            if (opt->te.time && timediff(str->time, opt->te) > -opt->ttol)
                break;
            ...
            sprintf(msg, "scanning: %s %s%s%s%s%s%s%s", time_str(str->time, 0), n[0] ? "G" : "", n[1] ? "R" : "", n[2] ? "E" : "", n[3] ? "J" : "",
                    n[4] ? "S" : "", n[5] ? "C" : "", n[6] ? "I" : "");
            if ((abort = showmsg(msg)))
                break;
        }
        close_strfile(str);
    }
...
static void outrnxevent(FILE *fp, const rnxopt_t *opt, gtime_t time, int event, const stas_t *stas, int staid) {
    ...
    if (event == EVENT_STARTMOVE) {
        ...
        fprintf(fp, "%-20.20s%-20.20s%-20.20s%-20s\n", p->sta.recsno, p->sta.rectype, p->sta.recver, "REC # / TYPE / VERS");
        fprintf(fp, "%-20.20s%-20.20s%-20.20s%-20s\n", p->sta.antsno, p->sta.antdes, "", "ANT # / TYPE");
        fprintf(fp, "%14.4f%14.4f%14.4f%-18s%-20s\n", p->sta.pos[0], p->sta.pos[1], p->sta.pos[2], "", "APPROX POSITION XYZ");
...
static int screent_ttol(gtime_t time, gtime_t ts, gtime_t te, double tint, double ttol) {
    ...
    return (tint <= 0.0 || fmod(time2gpst(time, NULL) + ttol, tint) <= ttol * 2.0) && (ts.time == 0 || timediff(time, ts) >= -ttol) &&
...
static void convnav(FILE **ofp, rnxopt_t *opt, strfile_t *str, int *n) {
    ...
    set = str->ephset;
    sys = satsys(sat, &prn);
    if (!(sys & opt->navsys) || opt->exsats[sat - 1])
        return;
    ...
    if (ts.time != 0)
        ts = timeadd(ts, -dtoe);
    if (!screent(str->time, ts, opt->te, 0.0))
        return;
...
static void convsbs(FILE **ofp, rnxopt_t *opt, strfile_t *str, int *n, gtime_t *tend) {
    ...
    if (!screent(time, opt->ts, opt->te, 0.0))
        return;
    ...
    *tend = time;
    ...
    if (!(sat = satno(sys, prn)) || opt->exsats[sat - 1] == 1)
        return;
...
static int convrnx_s(int sess, int format, rnxopt_t *opt, const char *file, char **ofile) {
    ...
    for (i = 0; i < nf && !abort; i++) {
        ...
        /* open stream file */
        if (!open_strfile(str, epath[i]))
            continue;

        /* input message */
        for (j = 0; (type = input_strfile(str)) >= -1; j++) {

            if (!(j % 11) && (abort = showstat(sess, str->time, str->time, n)))
                break;
    ...
    if (opt->ts.time == 0 || opt->te.time == 0 || opt->tunit <= 0.0) {
        ...
        for (i = 0;; i++) { /* for each session */
            ...
            if (timediff(opt_.te, opt->te) > 0.0)
                opt_.te = opt->te;
            opt_.tstart = opt_.tend = t0;
            if ((stat = convrnx_s(i + 1, format, &opt_, file, ofile)) < 0)
                break;
        }
    } else {
        showmsg("no period");
