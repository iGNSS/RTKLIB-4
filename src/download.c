...
static char *parse_str(char *buff, char *str, int nmax) {
    ...
    if (*p == '"')
        sep[0] = *p++; /* enclosed within quotation marks */
...
static int add_path(paths_t *paths, const char *remot, const char *dir) {
    ...
    if (!(paths->path[paths->n].remot = (char *) malloc(strlen(remot) + 1)) || !(paths->path[paths->n].local = (char *) malloc(strlen(local) + 1))) {
        return 0;
    }
...
static int gen_path(gtime_t time, gtime_t time_p, int seqnos, int seqnoe, const url_t *url, const char *sta, const char *dir, paths_t *paths) {
    ...
    if (strstr(url->path, "%N")) {
        for (i = seqnos; i <= seqnoe; i++) {
            ...
            if (time_p.time) {
                ...
                if (!strcmp(remot_p, remot))
                    continue;
            }
            if (!add_path(paths, remot, dir_t))
                return 0;
        }
    } else {
...
static int gen_paths(gtime_t time, gtime_t time_p, int seqnos, int seqnoe, const url_t *url, char **stas, int nsta, const char *dir, paths_t *paths) {
    int i;
...
static void compact_paths(paths_t *paths) {
    ...
    for (i = 0; i < paths->n; i++) {
        for (j = i + 1; j < paths->n; j++) {
            if (strcmp(paths->path[i].remot, paths->path[j].remot))
                continue;
...
static int get_list(const path_t *path, const char *usr, const char *pwd, const char *proxy) {
    FILE *fp;
...
static int rep_paths(path_t *path, const char *file) {
    ...
    if ((p = strrchr(buff1, '/')))
        ...
        p = buff1;
    if ((q = strrchr(buff2, FILEPATHSEP)))
        q++;
    else
        q = buff2;
    strcpy(p, file);
    ...
    if (!(remot = (char *) malloc(strlen(buff1) + 1)) || !(local = (char *) malloc(strlen(buff2) + 1))) {
        free(remot);
        return 0;
    }
    strcpy(remot, buff1);
...
/* execute download ----------------------------------------------------------*/
static int exec_down(path_t *path, char *remot_p, const char *usr, const char *pwd, const char *proxy, int opts, int *n, FILE *fp) {
    ...
    if (!strncmp(path->remot, "ftp://", 6))
        ...
        showmsg("STAT=X");
        if (fp)
            fprintf(fp, "%s ERROR (INVALID PATH)\n", path->remot);
        n[1]++;
        return 0;
    }
    /* test local file existence */
    ...
    /* get remote file list for FTP or FTPS */
    if ((proto == 0 || proto == 2) && (p = strrchr(path->remot, '/')) && strncmp(path->remot, remot_p, p - path->remot)) {
    ...
    if (!mkdir_r(dir)) {
        showmsg("STAT=X");
        if (fp)
            fprintf(fp, "%s -> %s ERROR (LOCAL DIR)\n", path->remot, dir);
        n[3]++;
        return 0;
    }
    ...
    /* proxy option */
    if (*proxy) {
        sprintf(env, "set %s_proxy=http://%s & ", proto == 0 || proto == 2 ? "ftp" : "http", proxy);
    ...
    if (proto == 0 || proto == 2) {
        sprintf(cmd,
                ...
                env, FTP_CMD, path->remot, usr, pwd, opt, FTP_RETRY, FTP_TIMEOUT, path->local, errfile, opt2);
    } else {
        if (*pwd) {
            sprintf(opt + strlen(opt), " --http-user=%s --http-password=%s ", usr, pwd);
        }
        sprintf(cmd, "%s%s %s %s-t %d -T %d -O \"%s\" -o \"%s\"%s\n", env, FTP_CMD, path->remot, opt, FTP_RETRY, FTP_TIMEOUT, path->local, errfile,
                opt2);
    }
    if (fp)
        fprintf(fp, "%s -> %s", path->remot, dir);
...
/* test local file -----------------------------------------------------------*/
static int test_local(gtime_t ts, gtime_t te, double ti, const char *path, const char *sta, const char *dir, int *nc, int *nt, FILE *fp) {
    gtime_t time;
...
/* test local files ----------------------------------------------------------*/
static int test_locals(gtime_t ts, gtime_t te, double ti, const url_t *url, char **stas, int nsta, const char *dir, int *nc, int *nt, FILE *fp) {
    int i;
...
static int print_total(const url_t *url, char **stas, int nsta, int *nc, int *nt, FILE *fp) {
    int i;
 ...
 *          not downloaded.
 *-----------------------------------------------------------------------------*/
extern int dl_exec(gtime_t ts, gtime_t te, double ti, int seqnos, int seqnoe, const url_t *urls, int nurl, char **stas, int nsta, const char *dir,
                   const char *usr, const char *pwd, const char *proxy, int opts, char *msg, FILE *fp) {
    paths_t paths = {0};
    gtime_t ts_p  = {0};
    ...
    for (i = 0; i < paths.n; i++) {
        ...
        sprintf(str, "%s->%s (%d/%d)", paths.path[i].remot, paths.path[i].local, i + 1, paths.n);
        if (showmsg(str))
            break;
    ...
    sprintf(msg, "OK=%d No_File=%d Skip=%d Error=%d (Time=%.1f s)", n[0], n[1], n[2], n[3], (tickget() - tick) * 0.001);
 ...
 * return : status (1:ok,0:error,-1:aborted)
 *-----------------------------------------------------------------------------*/
extern void dl_test(gtime_t ts, gtime_t te, double ti, const url_t *urls, int nurl, char **stas, int nsta, const char *dir, int ncol, int datefmt,
                    FILE *fp) {
    gtime_t time;
    double tow;
