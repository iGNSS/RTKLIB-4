...
static serial_t *openserial(const char *path, int mode, char *msg) {
    const int br[] = {300, 600, 1200, 2400, 4800, 9600, 19200, 38400, 57600, 115200, 230400, 460800, 921600};
    serial_t *serial;
    ...
    char dcb[64]    = "";
#else
    const speed_t bs[] = {B300, B600, B1200, B2400, B4800, B9600, B19200, B38400, B57600, B115200, B230400, B460800, B921600};
...
static int openfile_(file_t *file, gtime_t time, char *msg) {
    ...
    if (file->timetag) { /* output/sync time-tag */
        ...
        if (file->mode & STR_MODE_R) {
            if (fread(&tagh, TIMETAGH_LEN, 1, file->fp_tag) == 1 && fread(&time_time, sizeof(time_time), 1, file->fp_tag) == 1 &&
    ...
    if (file->fp_tag) {
        ...
        while ((int) (file->tick_n - t) <= 0) {
            ...
            if (fread(&file->tick_n, sizeof(tick), 1, file->fp_tag) < 1 ||
                fread((file->size_fpos == 4) ? (void *) &fpos_4B : (void *) &fpos_8B, file->size_fpos, 1, file->fp_tag) < 1) {
...
static udp_t *genudp(int type, int port, const char *saddr, char *msg) {
    ...
    if (!udp->type) { /* udp server */
        ...
        if (!strcmp(saddr, "255.255.255.255") && setsockopt(udp->sock, SOL_SOCKET, SO_BROADCAST, (const char *) &opt, sizeof(opt)) == -1) {
