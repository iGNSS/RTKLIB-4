...
static int is_obsmsg(int msg) {
    return (1001 <= msg && msg <= 1004) || (1009 <= msg && msg <= 1012) || (1071 <= msg && msg <= 1077) || (1081 <= msg && msg <= 1087) ||
           (1091 <= msg && msg <= 1097) || (1101 <= msg && msg <= 1107) || (1111 <= msg && msg <= 1117) || (1121 <= msg && msg <= 1127) ||
           (1131 <= msg && msg <= 1137);
}
/* test navigation data message ----------------------------------------------*/
static int is_navmsg(int msg) {
    return msg == 1019 || msg == 1020 || msg == 1044 || msg == 1045 || msg == 1046 || msg == 1042 || msg == 63 || msg == 1041;
}
...
static void write_obs(gtime_t time, stream_t *str, strconv_t *conv) {
    ...
    for (i = 0; i < conv->nmsg; i++) {
        ...
        if (conv->otype == STRFMT_RTCM2) {
            ...
            if (conv->msgs[i] <= 1012) {
                if (!gen_rtcm3(&conv->out, conv->msgs[i], 0, i != j))
                    continue;
                strwrite(str, conv->out.buff, conv->out.nbyte);
...
static void write_nav(gtime_t time, stream_t *str, strconv_t *conv) {
    ...
    for (i = 0; i < conv->nmsg; i++) {
        ...
        if (conv->otype == STRFMT_RTCM2) {
            ...
            if (!gen_rtcm3(&conv->out, conv->msgs[i], 0, 0))
                continue;
        } else
            continue;
...
static int nextsat(nav_t *nav, int sat, int msg) {
    ...
    for (p = p0 >= p2 ? p1 : p0 + 1; p != p0; p = p >= p2 ? p1 : p + 1) {
        ...
        if (sys == SYS_GLO) {
            ...
            sat = satno(sys, p);
            if (nav->eph[sat - 1 + MAXSAT * set].sat == sat)
                return sat;
        }
    }
    return 0;
 ...
 *-----------------------------------------------------------------------------*/
extern int strsvrstart(strsvr_t *svr, int *opts, int *strs, char **paths, char **logs, strconv_t **conv, char **cmds, char **cmds_periodic,
    ...
    /* open log streams */
    for (i = 0; i < svr->nstr; i++) {
        if (strs[i] == STR_NONE || strs[i] == STR_FILE || !*logs[i])
            continue;
