...
static int decode_galephemerisb(raw_t *raw) {
    ...
    if (!strstr(raw->opt, "-EPHALL")) {
        if (eph.iode == raw->nav.eph[sat - 1 + MAXSAT * set].iode && timediff(eph.toe, raw->nav.eph[sat - 1 + MAXSAT * set].toe) == 0.0 &&
...
static int decode_bdsephemerisb(raw_t *raw) {
    ...
    if (!strstr(raw->opt, "-EPHALL")) {
        if (timediff(raw->nav.eph[sat - 1].toe, eph.toe) == 0.0 && timediff(raw->nav.eph[sat - 1].toc, eph.toc) == 0.0)
            return 0;
    }
    raw->nav.eph[sat - 1] = eph;
    raw->ephsat           = sat;
