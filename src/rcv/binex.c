...
static const double ura_eph[] = {2.4, 3.4, 4.85, 6.85, 9.65, 13.65, 24.0, 48.0, 96.0, 192.0, 384.0, 768.0, 1536.0, 3072.0, 6144.0, 0.0};
...
static int decode_bnx_01_02(raw_t *raw, uint8_t *buff, int len) {
    ...
    if (!strstr(raw->opt, "-EPHALL")) {
        if (fabs(timediff(geph.toe, raw->nav.geph[prn - MINPRNGLO].toe)) < 1.0 && geph.svh == raw->nav.geph[prn - MINPRNGLO].svh)
            return 0;
    }
...
static int decode_bnx_01_03(raw_t *raw, uint8_t *buff, int len) {
    ...
    if (!strstr(raw->opt, "-EPHALL")) {
        if (fabs(timediff(seph.t0, raw->nav.seph[prn - MINPRNSBS].t0)) < 1.0 && seph.sva == raw->nav.seph[prn - MINPRNSBS].sva)
            return 0;
    }
...
static int decode_bnx_01_04(raw_t *raw, uint8_t *buff, int len) {
    ...
    if (!strstr(raw->opt, "-EPHALL")) {
        if (raw->nav.eph[sat - 1 + MAXSAT * set].iode == eph.iode && fabs(timediff(raw->nav.eph[sat - 1 + MAXSAT * set].toe, eph.toe)) < 1.0 &&
...
static uint8_t *decode_bnx_7f_05_obs(raw_t *raw, uint8_t *buff, int sat, int nobs, obsd_t *data) {
    ...
    const uint8_t codes_cmp[32] = {
        CODE_L2X, CODE_L2I, CODE_L2Q, CODE_L2X,
        CODE_L7X, CODE_L7I, /*  0- 5 */
        CODE_L7Q, CODE_L7X, CODE_L6X, CODE_L6I,
        CODE_L6Q, CODE_L6X, /*  6-11 */
        CODE_L1X, CODE_L1D, CODE_L1P, CODE_L1X,
        CODE_L5X, CODE_L5D, /* 12-17 */
        CODE_L5P, CODE_L5X, CODE_L7Z, CODE_L7D,
        CODE_L7P, CODE_L7Z /* 18-23 */
...
static int sync_bnx(uint8_t *buff, uint8_t data) {
    ...
    return buff[0] == BNXSYNC2 &&
           (buff[1] == 0x00 || buff[1] == 0x01 || buff[1] == 0x02 || buff[1] == 0x03 || buff[1] == 0x7D || buff[1] == 0x7E || buff[1] == 0x7F);
}
