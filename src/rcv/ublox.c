...
static int decode_rxmrawx(raw_t *raw) {
    ...
    for (i = 0, p += 16; i < nmeas && n < MAXOBS; i++, p += 32) {
        ...
        slip  = lockt == 0 || lockt * 1E-3 < raw->lockt[sat - 1][idx] || halfc != raw->halfc[sat - 1][idx] || (std_slip && cpstd >= std_slip);
...
static int decode_eph(raw_t *raw, int sat) {
    ...
    if (!strstr(raw->opt, "-EPHALL")) {
        if (eph.iode == raw->nav.eph[sat - 1].iode && eph.iodc == raw->nav.eph[sat - 1].iodc && timediff(eph.toe, raw->nav.eph[sat - 1].toe) == 0.0 &&
...
extern int gen_ubx(const char *msg, uint8_t *buff) {
    const char *cmd[]   = {"PRT", "USB",  "MSG",  "NMEA", "RATE",   "CFG",    "TP",  "NAV2",   "DAT",    "INF",       "RST",    "RXM",   "ANT",
    ...
    const uint8_t id[]  = {0x00, 0x1B, 0x01, 0x17, 0x08, 0x09, 0x07, 0x1A, 0x06, 0x02, 0x04, 0x11, 0x13, 0x0E, 0x16, 0x80, 0x10, 0x19, 0x1D,
