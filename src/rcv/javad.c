...
static int decode_eph(raw_t *raw, int sys) {
    ...
    if (!strstr(raw->opt, "-EPHALL")) {
        if (timediff(raw->nav.eph[sat - 1 + MAXSAT * set].toe, eph.toe) == 0.0 && raw->nav.eph[sat - 1 + MAXSAT * set].iode == eph.iode &&
...
static int sync_javad(uint8_t *buff, uint8_t data) {
    ...
    return (p == '\r' || p == '\n') && ISTXT(buff[0]) && ISTXT(buff[1]) && ISHEX(buff[2]) && ISHEX(buff[3]) && ISHEX(buff[4]);
}
/* clear buffer --------------------------------------------------------------*/
