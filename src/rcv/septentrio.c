...
static int decode_glorawca(raw_t *raw) {
    ...
    if (!strstr(raw->opt, "-EPHALL")) {
        if (geph.iode == raw->nav.geph[prn - 1].iode && timediff(geph.toe, raw->nav.geph[prn - 1].toe) == 0.0)
            return 0;
    }
...
static int decode_galrawfnav(raw_t *raw) {
    ...
    if (!strstr(raw->opt, "-EPHALL")) {
        if (eph.iode == raw->nav.eph[sat - 1 + MAXSAT].iode && timediff(eph.toe, raw->nav.eph[sat - 1 + MAXSAT].toe) == 0.0 &&
