...
static void outrpos(FILE *fp, const double *r, const solopt_t *opt) {
    ...
    if (opt->posf == SOLF_LLH || opt->posf == SOLF_ENU) {
        ...
        if (opt->degf) {
            ...
            fprintf(fp, "%3.0f%s%02.0f%s%08.5f%s%4.0f%s%02.0f%s%08.5f%s%10.4f", dms1[0], sep, dms1[1], sep, dms1[2], sep, dms2[0], sep, dms2[1], sep,
                    dms2[2], sep, pos[2]);
        } else {
...
static void update_rtcm_ssr(gtime_t time) {
    ...
    while (timediff(rtcm.time, time) < 1E-3) {
        ...
        for (i = 0; i < MAXSAT; i++) {
            if (!rtcm.ssr[i].update || rtcm.ssr[i].iod[0] != rtcm.ssr[i].iod[1] || timediff(time, rtcm.ssr[i].t0[0]) < -1E-3)
                continue;
...
static int readobsnav(gtime_t ts, gtime_t te, double ti, char **infile, const int *index, int n, const prcopt_t *prcopt, obs_t *obs, nav_t *nav,
                      sta_t *sta) {
    ...
    for (i = 0; i < n; i++) {
        ...
        if (readrnxt(infile[i], rcv, ts, te, ti, prcopt->rnxopt[rcv <= 1 ? 0 : 1], obs, nav, rcv <= 2 ? sta + rcv - 1 : NULL) < 0) {
...
static int avepos(double *ra, int rcv, const obs_t *obs, const nav_t *nav, const prcopt_t *opt) {
    ...
    for (iobs = 0; (m = nextobsf(obs, &iobs, rcv)) > 0; iobs += m) {
        ...
        if (!pntpos(data, j, nav, opt, &sol, NULL, NULL, msg))
            continue;
...
static int execses(gtime_t ts, gtime_t te, double ti, const prcopt_t *popt, const solopt_t *sopt, const filopt_t *fopt, int flag, char **infile,
                   const int *index, int n, char *outfile) {
    FILE *fp;
    prcopt_t popt_ = *popt;
...
static int execses_r(gtime_t ts, gtime_t te, double ti, const prcopt_t *popt, const solopt_t *sopt, const filopt_t *fopt, int flag, char **infile,
                     const int *index, int n, char *outfile, const char *rov) {
    gtime_t t0 = {0};
    int i, stat = 0;
...
static int execses_b(gtime_t ts, gtime_t te, double ti, const prcopt_t *popt, const solopt_t *sopt, const filopt_t *fopt, int flag, char **infile,
                     const int *index, int n, char *outfile, const char *rov, const char *base) {
    gtime_t t0 = {0};
    int i, stat = 0;
 ...
 *-----------------------------------------------------------------------------*/
extern int postpos(gtime_t ts, gtime_t te, double ti, double tu, const prcopt_t *popt, const solopt_t *sopt, const filopt_t *fopt, char **infile,
    ...
    if (!openses(popt, sopt, fopt, &navs, &pcvss, &pcvsr))
        return -1;
        ...
        for (i = 0;; i++) { /* for each periods */
            ...
            for (j = k = nf = 0; j < n; j++) {
                ...
                if (ext && (!strcmp(ext, ".rtcm3") || !strcmp(ext, ".RTCM3"))) {
                    ...
                    ttte = tte;
                    if (ext && (!strcmp(ext, ".sp3") || !strcmp(ext, ".SP3") || !strcmp(ext, ".eph") || !strcmp(ext, ".EPH"))) {
